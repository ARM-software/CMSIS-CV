...
# Version: 3.0
# Date: 2023-11-06
# This bash script generates CMSIS-CV Documentation:
#
# Pre-requisites:
...
projectName=$(grep -E "PROJECT_NAME\s+=" cv.dxy.in | sed -r -e 's/[^"]*"([^"]+)".*/\1/')
projectNumberFull="${VERSION_FULL}"
projectNumber="${projectNumberFull%+*}"
datetime=$(date -u +'%a %b %e %Y %H:%M:%S')
year=$(date -u +'%Y')

sed -e "s/{projectNumber}/${projectNumber}/" cv.dxy.in \
  | sed -e "s/{cmsisProjectNumber}/${cmsisProjectNumber}/" \
  > cv.dxy

git_changelog -f html -p "v" > src/history.txt

echo_log "\"${UTILITY_DOXYGEN}\" cv.dxy"
"${UTILITY_DOXYGEN}" cv.dxy
